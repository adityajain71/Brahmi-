...
// Copied from original index.ts
function buildSwarData(language) {
    ...
    practice_time: {
      ...
      sections: {
        ...
        section_12_writing_practice_intro: {
          title_hindi: 'आइए! अब लिखने का अभ्यास অনুশীল کرتے हैं।', // oops weird text
...
const hindiData = buildSwarData('hindi');
const hindiSwarPath = path.join(__dirname, '../backend/data/hindi/swar.json');
const swarJson = JSON.parse(fs.readFileSync(hindiSwarPath, 'utf8'));

// merge
Object.assign(swarJson, hindiData);

fs.writeFileSync(hindiSwarPath, JSON.stringify(swarJson, null, 2));
console.log('Hindi swar.json populated with buildSwarData.');
