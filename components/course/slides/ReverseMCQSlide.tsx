...
function buildQuestion(example: MCQExample, allForms: MatraCombinationForm[]) {
  const correct = example.answer // Devanagari
  const distractors = allForms
    .filter(f => f.combinedDevanagari !== correct && f.combinedDevanagari !== example.prompt)
    .sort(() => Math.random() - 0.5)
    .slice(0, 3)
    .map(f => f.combinedDevanagari)
    ...
    >
      ...
      <AnimatePresence mode="wait">
        {allDone ? (
          ...
          >
            {/* Brahmi prompt */}
            <div className="w-36 h-36 md:w-44 md:h-44 rounded-3xl bg-[#1a1613] border-2 border-[#D4AF37]/40 flex items-center justify-center shadow-[0_0_25px_rgba(212,175,55,0.15)]">
              <span
                className="text-5xl md:text-6xl text-[#FFD6A5] leading-none drop-shadow-[0_0_8px_rgba(212,175,55,0.4)]"
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`py-6 rounded-2xl border-2 text-2xl md:text-3xl font-serif font-bold transition-all duration-200 flex items-center justify-center ${cls}`}
                  >
                    {opt}
                  </motion.button>
                )
            ...
            <AnimatePresence>
              {selected && selected !== q.correct && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="flex items-center gap-2 bg-[#2a2420] border border-[#D4AF37]/20 rounded-2xl px-5 py-3 w-full"
                >
