...
const getLocalizedLabel = (c, lang) => {
...
if "DEVANAGARI_TO_TAMIL_MAP" not in text:
    text = text.replace("const DEVANAGARI_TO_TAMIL_LETTER_MAP = {", injection + "\nconst DEVANAGARI_TO_TAMIL_LETTER_MAP = {")

# find translateVyanjanArray
start_idx = text.find("const translateVyanjanArray = (vyanjan, lang) => {")
end_idx = text.find("const translateMatraArray = (matra, lang) => {")

if start_idx == -1 or end_idx == -1:
    print("Could not find functions")
    exit(1)

new_func = """const translateVyanjanArray = (vyanjan, lang) => {
  return vyanjan.map(slide => {
    let s = clone(slide);

    if (s.consonant) {
        s.localizedLabel = getLocalizedLabel(s.consonant, lang);
    }
    ...
    const knLetter = meta.title_kannada || c;
    const taLetter = DEVANAGARI_TO_TAMIL_LETTER_MAP[c] || c;
    ...
    if (lang === 'en') {
      ...
      exStr = (meta.exampleWords || []).map(ex => ex.kannada || ex.devanagari).join(', ');
    } else if (lang === 'ta') {
      exStr = (meta.exampleWords || []).map(ex => ex.tamil || ex.devanagari).join(', ');
    }
    ...
    if (s.type === 'bonus_title') {
      ...
      if (lang === 'en') {
        ...
        s.content = `ಬ್ರಾಹ್ಮಿ ಲಿಪಿಯಲ್ಲಿ "${locLetter}" ದ ರೂಪ: ${brahmi} ಆಗಿದೆ. ಉಚ್ಚಾರಣೆ: ${meta.pronunciationNote || ''} (ಉದಾಹರಣೆಗೆ – ${exStr})`;
      } else if (lang === 'ta') {
      ...
      if (lang === 'en') {
        ...
        formsStr = forms.map(f => f.combinedDevanagari).join(' – ');
      } else if (lang === 'ta') {
        formsStr = forms.map(f => f.combinedDevanagari).join(' – ');
      }
    ...
    if (s.uxNote) {
      const uxMap = {
        en: 'Read and proceed',
        kn: 'ಓದಿ ಮತ್ತು ಮುಂದುವರಿಯಿರಿ',
        ta: 'படித்துத் தொடரவும்'
      };
      s.uxNote = uxMap[lang] || s.uxNote;
    }
